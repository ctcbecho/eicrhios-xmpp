...
public class Terminal {
	...
	private int[] vt100_charset_graph = new int[] { 0x25ca, 0x2026, 0x2022,
			0x3f, 0xb6, 0x3f, 0xb0, 0xb1, 0x3f, 0x3f, 0x2b, 0x2b, 0x2b, 0x2b,
			0x2b, 0xaf, 0x2014, 0x2014, 0x2014, 0x5f, 0x2b, 0x2b, 0x2b, 0x2b,
			0x7c, 0x2264, 0x2265, 0xb6, 0x2260, 0xa3, 0xb7, 0x7f };
	...
	private void vt100_setmode(String p, boolean state) {
		...
		for (String m : ps) {
			...
			if ("4".equals(m)) {
				...
				if ((state && !vt100_mode_alt_screen)
						|| (!state && vt100_mode_alt_screen)) {
					int[] s = screen;
					screen = screen2;
					screen2 = s;
					...
					if (vt100_parse_len > 32) {
						...
						if (msb == 0x20) {
							...
							vt100_parse_func += (char) c;
								...
								&& vt100_parse_state == State.Csi) {
							vt100_parse_param += new String(
									new char[] { (char) c });
						} else {
							vt100_parse_func <<= 8;
							vt100_parse_func += (char) c;
	...
	public synchronized String dump(long timeout, boolean forceDump)
